...
def parse_args():
    ...
    parser.add_argument('--cpdlist_file', default='FRoGS/data/compound_list_shRNA.txt',
                        help='Path to a file that lists the compounds for cross validation')
    parser.add_argument('--target_file', default='FRoGS/data/cpd_gene_pairs.csv',
                        help='Path to a file containing target annotations of compounds')
    parser.add_argument('--sig_file', default='FRoGS/data/L1000_PhaseI_and_II.csv',
    ...
    parser.add_argument('--emb_go', default='FRoGS/data/gene_vec_go_256.csv',
                        help='Gene embeddings learned from GO annotations')
    parser.add_argument('--emb_archs4', default='FRoGS/data/gene_vec_archs4_256.csv',
                        help='Gene embeddings learned from archs4 gene expression experiments')
    parser.add_argument('--epochs', default=60,
                        help='Number of training epochs')
    parser.add_argument('--outdir', default='FRoGS/results/',
                        help='Path to a directory to save prediction lists')
    parser.add_argument('--modeldir', default='FRoGS/saved_model/',
...
def gene_sim_mean_std_celltypes():
    '''Compute the mean and standard diviation of each gene's cosine
       similarities with all the genes in the dataset
       return: a mapping from each gene ID to the mean of its
               similarities with all the genes in the dataset;
    ...
    '''
    all_genes = set()
    for emb in celltypes_gene_embs.values():
        all_genes.update(list(emb.keys()))
    all_genes.update(list(archs4_emb.keys()))
    # all_genes=set(list(go_emb.keys()) + list(archs4_emb.keys()))
    ...
    celltypes_mean_std_dict = {}
    for celltype, emb in celltypes_gene_embs.items():
        celltypes_mean_std_dict[celltype] = sim_mean_std(emb)

    # return sim_mean_std(go_emb), sim_mean_std(archs4_emb)
    return celltypes_mean_std_dict, sim_mean_std(archs4_emb)

def get_vec_celltype(X):
    sig2vec = {}
    l1k, S_hit, celltype = X
    vec = compute_list_emb_celltype(S_hit, celltypes_gene_embs[celltype], celltypes_mean_std_dict[celltype])
    sig2vec[l1k] = vec
    return sig2vec
...
def prepare_data(cpd_train, prt):
    ...
    for k,cpd in enumerate(tqdm(cpd_train, desc="Positive compound target pairs")):
    ...
    for cl in tqdm(cl_all, desc="Sampling negative compound target pairs"):
    ...
    for k,cpd in enumerate(cpd_test):
        ...
        for cl in cpd_sig_cl_df:
            ...
            if len(set(cpd2target[cpd]).intersection(pert_sig.loc[tgtcl_idx[cl], 'Name'].tolist())) > 0:
                ...
                scores = classifier.predict([test_input_l, test_input_r], verbose=0)
                ...
                print(f"There are {len(genes)} candidate genes")
...
# Modified by MT on 2024-12-21
def compute_list_emb_celltype(glist, celltype_dependent_emb, celltype_dependent_mean_std_dict_go):
    ...
    for i,gid in enumerate(glist):
        if gid in celltype_dependent_emb:
            go_mat[i,:]=celltype_dependent_emb[gid]
        else:
            go_mat[i,:]=np.zeros(len(list(celltype_dependent_emb.values())[0]))
    ...
    sim_mean_go=np.array([celltype_dependent_mean_std_dict_go[gid][0] if gid in celltype_dependent_mean_std_dict_go else 0 for gid in glist])
    sim_std_go=np.array([celltype_dependent_mean_std_dict_go[gid][1] if (gid in celltype_dependent_mean_std_dict_go and celltype_dependent_mean_std_dict_go[gid][1] > 0) else float('inf') for gid in glist])
...
celltypes = open("../../data/fig5/FRoGS_celltypes.txt").read().splitlines()
celltypes_gene_embs = {}
for celltype in celltypes:
    in_path = f"../../data/fig5/GeneRelNet-{celltype}/gene_vec_DPWalk_256.csv"
    with open(in_path, mode='r') as infile:
        reader = csv.reader(infile)
        emb = {rows[0]:np.array(rows[1:], dtype=np.float32) for rows in reader}
    celltypes_gene_embs[celltype] = emb

# mean_std_dict_go, mean_std_dict_archs4 = gene_sim_mean_std()
celltypes_mean_std_dict, mean_std_dict_archs4 = gene_sim_mean_std_celltypes()
...
# SNUC4 seems not included in LINCS cell information
# Because SNUC5 is almost the same as SNUC4, replace it with SNUC5
t_sig.loc[t_sig['term_name'].str.contains('SNUC4'), 'term_name'] = t_sig.loc[t_sig['term_name'].str.contains('SNUC4'), 'term_name'].str.replace('SNUC4', 'SNUC5')
...
for idx in t_sig.index:
    ...
    celltype = t_sig.loc[idx, "term_name"].split("@")[1].split(":")[0]

    if name in cpd2target:
        pert_sig.append([l1k, name, cl, dir, des, ty, S_hit])
        id2sig[l1k] = S_hit
        # tasks.append((l1k, S_hit))
        tasks.append((l1k, S_hit, celltype))
        all_cl.add(cl)

id_map=pd.read_csv('FRoGS/data/term2gene_id.csv')
term2geneid = {}
...
print("Number of unique cell lines used:", pert_sig['CellLine'].nunique())
compound_names_used = set(pert_sig[pert_sig['Perturbagen'] == 'Cpd']['Name'].unique())
print("Number of compounds used:", len(compound_names_used))
gene_names_used = set(pert_sig[pert_sig['Perturbagen'].isin(['shRNA', 'cDNA'])]['Name'].unique())
print("Number of target genes used:", len(gene_names_used))
compound_gene_pairs = sum([len(v) for k, v in cpd2target.items() if k in compound_names_used])
print("Number of compound-target gene pairs:", compound_gene_pairs)
...
bs = len(tasks) // 20 + 1
rslt_nested = Parallel(n_jobs=10)(delayed(lambda Xs: [get_vec_celltype(X) for X in Xs])(tasks[i:i+bs]) for i in tqdm(range(0, len(tasks), bs)))
rslt = []
for lst in rslt_nested:
    rslt.extend(lst)
del rslt_nested
