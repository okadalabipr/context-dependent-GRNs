...
def get_transition_probs_multisteps(graph, steps=3):
    mat = nx.adjacency_matrix(graph, weight="weight").toarray().astype("float")
    mat /= mat.sum(axis=1, keepdims=True)

    multistep_probs = np.zeros_like(mat)
    for i in range(1, steps+1):
        multistep_probs += np.linalg.matrix_power(mat, i)
...
if __name__ == "__main__":
    ...
    os.makedirs(f"../data/fig5/{config.exp}", exist_ok=True)

    ####################
    ### Load network
    ####################
    network = pd.read_parquet("../data/fig5/STRING_network.parquet")

    # Create a graph
    edges = [(pair.split("--")[0], pair.split("--")[1], count) for pair, count in zip(network["pair"], network["combined_score"])]
    G = nx.Graph()
    ...
    ### Multistep probs
    ####################
    multistep_probs = get_transition_probs_multisteps(main_subgraph, steps=config.steps)

    ###########################
    # Dataset
    ###########################
    class GeneDataset(Dataset):
        def __init__(self, gene_ids, multistep_probs, neg_samples):
            self.gene_ids = gene_ids
            self.multistep_probs = multistep_probs
            self.neg_samples = neg_samples
            self.pos_samples = 1
        def __len__(self):
            return len(self.gene_ids)
        def __getitem__(self, idx):
            p = self.multistep_probs[idx].copy()
            if p[idx] != 1:
                p[idx] = 0
            p = p / np.sum(p)
            posid = np.random.choice(len(self.gene_ids), self.pos_samples, p=p)

            diffusion = self.multistep_probs[idx].copy()
    ...
    ###########################
    ds = GeneDataset(gene_ids, multistep_probs, config.num_negative_samples)
    ...
    fw = open(f"../data/fig5/{config.exp}/gene_vec_STRING_{config.embedding_dim}.csv", 'w')
