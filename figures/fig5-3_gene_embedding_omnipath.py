...
if __name__ == "__main__":
    ...
    ### Load network
    ####################
    network = pd.read_parquet("../data/fig5/omnipath_interactions_unique_pairs.parquet")
    network["weight"] = 1

    # Create a graph
    edges = [(pair.split("--")[0], pair.split("--")[1], count) for pair, count in zip(network["pair"], network["weight"])]
    G = nx.Graph()
    ...
    fw = open(f"../data/fig5/{config.exp}/gene_vec_omnipath_{config.embedding_dim}.csv", 'w')
