...
if __name__ == "__main__":
    ...
    ####################
    config = Namespace(
        exp = "GeneRelNet-context-independent",
        seed=42,
        # DeepWalk parameters
        embedding_dim=256,
    ...
    config.exp = config.exp + f"_beta{config.beta}_steps{config.steps}_neg{config.num_negative_samples}"
    ...
    relations_df["count"] = 1
    print("Number of interactions:", len(relations_df))

    # Calculate context-dependent weights
    relations_df["pair"] = relations_df.apply(lambda x: "--".join(sorted([str(x["from_entrez"]), str(x["to_entrez"])])), axis=1)
    relations_df = relations_df.drop_duplicates(subset=["pmid", "pair"]).reset_index(drop=True)

    relations_df = relations_df[["pair", "count"]].groupby("pair").agg({"count": "sum"}).reset_index()
    # normalize
    relations_df["count"] = relations_df["count"] / relations_df["count"].sum() * 1e6
    # Log transformation to make the distribution more normal
    relations_df["count"] = np.log1p(relations_df["count"].values)
    # rename
    ...
    fw = open(f"../data/fig5/{config.exp}/gene_vec_DPWalk_{config.embedding_dim}.csv", 'w')
