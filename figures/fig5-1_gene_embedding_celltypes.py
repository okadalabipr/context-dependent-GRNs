...
if __name__ == "__main__":
    ...
    config.exp = config.exp + f"-{celltype}"
    ...
    threshold = 0.1
    pmid_corrs = pd.read_parquet("../data/fig5/cellline_pmid_bert_corrs.parquet", columns=["pmid", celltype])

    relations_df["corr"] = relations_df["pmid"].map(pmid_corrs.set_index("pmid")[celltype])
